...
class Test: EventClient
{
    ...
    char[] command(char[] str)
    {
    	...
    	{
        	case "test":
            		return "TEST command executed"; // no need to break as we return anyways
            case "":
                    return "";
        	default:
    ...
    void evStart()
    {
        ...
        console.command = &vm.command;
        console.autocomplete = &vm.autocomplete;
    ...
    void evDraw()
    {
        ...
        monoFont.printT(Vector(0, 120), "Finally got text working!\nTesting for UTF:\nČŠŽ čšž ░▒▓\nNOTE: Might not work if the font does not support UTF");
...
int main(char[][] args)
{
    ...
    derelict.util.loader.Derelict_SetMissingProcCallback(&cbMissingProc);

    new SiegeModule("GLFW");
    new SiegeModule("OpenGL");
    new SiegeModule("DevIL");
    new SiegeModule("OpenAL");
    new SiegeModule("SndFile");
    new SiegeModule("Freetype");
    new SiegeModule("Chipmunk");
